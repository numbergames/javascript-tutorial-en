...
Some simple regular expressions can have veeeeeery long execution times, and even "hang" the JavaScript engine.

Sooner or later all developers eventually meet this behavior.
...
That may even be a vulnerability. For instance, if JavaScript is on the server and uses regular expressions on user data. There have been many vulnerabilities of that kind found even in widely distributed systems.

So the problem is definitely worth understanding.
...
That doesn't yet support the details of HTML grammar, for instance strings can be in 'single' quotes, but these can be added later, so that's somewhat close to real life. For now we want the regexp to be simple.
...
What's the matter? Why would a simple regular expression on such a small string "hang"?
...
In the example above, when we search `pattern:<(\s*\w+=\w+\s*)*>` in the string `subject:<a=b  a=b  a=b  a=b` -- we see problem again.

The string has no `>` at the end, so the match is impossible, but the regexp engine cannot see that until search backtracks trying different combinations of `pattern:(\s*\w+=\w+\s*)`:
...
Decreasing the count of `pattern:\d+` doesn't help to find a match, there's no difference between these two:
...
Let's get back to a more real-life example: `pattern:<(\s*\w+=\w+\s*)*>`. We want it to find pairs `name=value` (as many as it can). There's no advantage to backtracking here.
...
Modern regexp engines support so-called "possessive" quantifiers for that. They are greedy, but without the backtracking. Pretty simple, they capture whatever they can, and the search continues. There's also another tool called "atomic groups" that forbid backtracking inside parentheses.

Unfortunately, neither of these features are supported by JavaScript's regexp standard.
...
The pattern to take as many repetitions as possible without backtracking is: `pattern:(?=(a+))\1`.
